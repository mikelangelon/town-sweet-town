...
func (ts *PeopleScene) Draw(screen *ebiten.Image) {
	ts.MapScene.Draw(screen)
	ts.state.Player.Draw(screen)
}

func (ts *PeopleScene) Update() error {
	...
	for k, v := range ts.TransitionPoints {
		if x >= k.X && x < k.X+16 &&
			y >= k.Y && y < k.Y+16 {
			ts.sm.SwitchWithTransition(v, stagehand.NewTicksTimedSlideTransition[State](stagehand.LeftToRight, time.Second*time.Duration(1)))
			return nil
		}
	}
	return nil
...
func (ts *PeopleScene) Load(st State, sm stagehand.SceneController[State]) {
